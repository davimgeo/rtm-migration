...
class Model:
  def __init__(self, c: Config, geom: Geometry) -> None:
    ...
    self.model = cp.zeros(shape, dtype=cp.float32)
    self.model_smooth = cp.zeros(shape, dtype=cp.float32)
  ...
  def load(self) -> None:
    self.model = cp.fromfile(
      self.c.model_path, dtype=cp.float32, count=self.c.nx*self.c.nz
  ...
  def set_boundary(self) -> None:
    model_ext = cp.zeros((self.nzz, self.nxx))
  ...
  def __gaussian_filter(
      ...
      x = cp.arange(-radius, radius + 1)
      gaussian = cp.exp(-0.5 * (x**2) / (sigma**2))
      ...
      for col in range(arr.shape[1]):
        ...
        padded = cp.pad(data, radius, mode="reflect")

        smoothed = cp.convolve(padded, gaussian, mode="same")
  ...
  def plot(self, model=None):
      if model is None:
        model = cp.asarray(self.model)

      xloc = cp.linspace(0, self.c.nx - 1, 11, dtype=int)
      xlab = cp.array(xloc * self.c.dh, dtype=int)

      zloc = cp.linspace(0, self.c.nz - 1, 7, dtype=int)
      zlab = cp.array(zloc * self.c.dh, dtype=int)
